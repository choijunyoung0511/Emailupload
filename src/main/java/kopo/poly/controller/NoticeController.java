...
public class NoticeController {
    ...
    public MsgDTO noticeInsert(HttpServletRequest request, HttpSession session, Model model) {
        log.info("if.noticeInsert start!", this.getClass().getName());
        String msg = "";
        MsgDTO dto;
        try {
            String userId = kopo.poly.util.CmmUtil.nvl((String) session.getAttribute("SESSION_USER_ID"));
            String title = kopo.poly.util.CmmUtil.nvl(request.getParameter("title"));
            String noticeYn = kopo.poly.util.CmmUtil.nvl(request.getParameter("noticeYn"));
            String contents = kopo.poly.util.CmmUtil.nvl(request.getParameter("contents"));
    ...
    public MsgDTO noticeUpdate(HttpSession session, HttpServletRequest request) {
        ...
        MsgDTO dto;
         try {
             ...
             log.info("user_Id : {} / nSeq : {} / title : {} / noticeYn : {} / contents : {}", user_id, nSeq, title, noticeYn, contents);;
             NoticeDTO pDTO = new NoticeDTO();
